...
现在我们明白了为什么需要这样一个开发服务器,问题是开发流程是长什么样子的?或者说它在开始的时候是怎么样的?直到现在,我们的开发流程基本上是在顶部放置脚本导入来导入Vue包,因为我们带入了Vue脚本,然后就可以写Vue代码了,这样就搭建好了开发环境,这种方式在简单的应用中是可行的.

在更复杂的应用中,你可能由大量脚本需要导入,而且你可能要用大量的包,你想把它们打包在一起并且构建它们,你可能想使用我之前讲过的ES6,所以你可能不想只用ES5,你还想用ES6,并且能够让两种都能编译,这是我们写的代码,当交付到用户可以看到的生产服务器上时,我们想在这之前转换代码,应用一些特殊功能,使用一些开发流程无法提供的特殊功能.

这些特殊功能使我们可以在Vue应用中编译单文件模板,我们到现在为止还没有讲过单文件模板,单文件模板是一种有效的解决方案,它用el属性从DOM上关联一个模板或者是template属性,我们会在接下来的课程中只用单文件模板,我们深入使用它并解释它们是怎么运作的,所以我们有了这个单文件模板,这样称呼是因为模板被分到一个单独的文件,流程中会用到一些特定的工具,这些工具可以识别单文件模板并能编译它们,我的意思是在开发流程的过程中编译,所以我们交付的应用已经有了编译好的模板,即模板变成了在浏览器中可执行的网页代码,这种方法和我在上一章教的不一样.

我们目前使用的配置中所有代码都由浏览器编译,我们交付了字符串或者这些元素选择器,接着在运行时让Vue在DOM里选择这些部分,然后在哪里编译成原生DOM,用这种方式在机器上编译,可以让应用变得更快和更小,因为这样可以摆脱编译器,然后交付最后的代码,因此需要这样一个开发流程,不能绕过它,这种方式还允许我们使用不区分大小写的组件选择器.

现在回到组件部分,每一个组件都可以应用预处理器,例如:可以使用ES6代码,或者如果你想也可以把Sass编译成CSS,就像之前说的一样,拥有这些特殊功能不仅可以让我们写代码更有趣,还带来了一些额外的好处,例如:允许我们使用下一代JavaScript特性,允许我们使用这种单文件模板,还可以压缩发送到服务器上Vue代码,可以摆脱那些编译器,因为在浏览器里用不到他,当然,重新渲染的情况经常出现,但是已经有了编译或者说是转换成了JavaScript模板,所以我们有了这个附带效果,这些都是我们用开发流程的原因,它可以让我们写出更好的代码,允许我们用其他特性,减少打包后的应用的大小等,我们在构建大型和中型应用时,就需要这些功能,

### 6.4 使用Vue命令行来创建项目

在上一节中,我们提到可能用到开发工作流的原因,以及它到底什么?

我们可以自己设置一个这样的工作流,如果对webpack熟悉的话,可以使用它,vue-loader可以立刻保存工作流,它是一个包,允许我们转化单一文件模板等,但是我们不必亲自完成,在本节课我们也不会这么做,我们会用到vue-cli,vue-cli听起来像强大的多功能构建工具,实际上vue-cli有一个最中心的任务,它可以帮助我们获取Vue项目的模板,即获取某种空白的项目配置,但是配置好了构建过程,我们可以深入了解构建过程,再稍微调整下来满足个人需求,或者学会如何设置自己的构建过程,通过命令安装vue-cli是很容易的,我们可以访问大量可用模板,比如: simple模板,这基本上与使用本地一样,这是一个HTML文件,需要使用CDN输出的脚本,同时还有webpack-simple工作流,这将是本节课用到的,它包括很好用的webpack工作流,可以重新编译单一文件模板,这样我们就可以使用ES6,这些功能囊括了我们所有的需求,十分实用,
帮助我们构建适合生产应用程序.

如果你想要用包含测试以及其他高级功能的大包,可以使用webpack工作流,它的功能更加强大,相比webpack,你如果更喜欢browserify,也可以用它,但是我们之所以使用webpack,因为它是目前比较出色的web开发工具,至于之前提到的模板编译,除了simple之外的所有模板都支持这个,那么就可以把单文件模板编译到JavaScript代码,这样我们就可以拥有更好的运行时配置,生成更小的Vue包,实现性能的提升,

### 6.5 Vue命令行安装以及创建一个新项目

1. 先安装npm,npm是一个node包管理器,所以为了让这个命令能够运行,就必须先安装nodeJS,你不必了解NodeJS,因为我们不是去写NodeJS,我们不是要写Node代码,不是要自己写服务端代码,我们现在只需要node包管理器,一个用来管理前端开发依赖的工具,所以我们肯定要安装他,而且我们使用的模板已经包含了开发服务器,所以我们不必自己写,但他仍然需要NodeJS来运行,所以我们也需要NodeJS,下载安装好之后,就能使用Npm然后将NodeJS作为服务器.
2. 在全局安装vue-cli命令

	npm install -g vue-cl

3. 进入到想存储项目的目录,或创建vue-cli项目和VueJS项目的文件夹了,接下来通过简单的运行vue,因为刚刚安装了vue-cli所以这个命令已经可用了,然后接上项目要使用的模板,接下来我们再看一下vue-cli的Github页面,你可以通过百度vue-cli找到他,然后就可以看到一些说明,选择webpack-simple这个模板,进行安装:

	vue init 模板名称 项目名称

4. 接下来进入项目文件夹,`cd 后面跟的是项目文件夹的名称,`,然后运行npm install 拉取项目所需要的依赖
